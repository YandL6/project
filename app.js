...
const DATA_VERSION = 64; // 默认数据版本，更新默认数据时 +1 // 默认数据版本，更新默认数据时 +1
...
const DEFAULT_PROBLEMS = [
  ...
  },
  {
    ...
    knowledge: ['深度优先搜索', '广度优先搜索', '并查集', '矩阵操作'],
  ...
  },
  {
    ...
    knowledge: ['哈希表', 'LRU 设计模式', 'list', '链表'],
...
const DEFAULT_KNOWLEDGE = [
  ...
  {
    id: 38, name: '并查集', category: '数据结构',
    summary: 'Union-Find：用森林维护不相交集合，find 查根 + union 合并，路径压缩 + 按秩合并后单次操作均摊 O(α(n))≈O(1)',
    content: '并查集（Disjoint Set Union，DSU）维护若干个不相交的集合，支持两种操作：\n- find(x)：查询 x 属于哪个集合（返回该集合的根/代表元）\n- union(a, b)：把 a 和 b 所在的两个集合合并成一个\n\n底层结构：parent 数组构成一片森林，parent[x] 存 x 的父节点，根节点 parent[x] = x。同一棵树 = 同一个集合。初始化时每个元素自成一棵单节点树。\n\n核心代码模板（刷题直接背）：\n  vector<int> parent, rnk;\n  void init(int n) {\n      parent.resize(n); rnk.assign(n, 0);\n      iota(parent.begin(), parent.end(), 0);   // parent[i] = i\n  }\n  int find(int x) {\n      return parent[x] == x ? x : parent[x] = find(parent[x]);  // 路径压缩\n  }\n  bool unite(int a, int b) {\n      a = find(a); b = find(b);\n      if (a == b) return false;        // 已同集合，合并失败（可用于判成环）\n      if (rnk[a] < rnk[b]) swap(a, b); // 按秩合并：矮树挂高树\n      parent[b] = a;\n      if (rnk[a] == rnk[b]) rnk[a]++;\n      return true;\n  }\n  bool connected(int a, int b) { return find(a) == find(b); }\n\n两个关键优化：\n- 路径压缩：find 递归回溯时把沿途节点全部直接挂到根上，下次查询一步到位。模板里 parent[x] = find(parent[x]) 一行同时完成递归与压缩\n- 按秩合并（rank 即树高的上界）：合并时把矮树挂到高树下，树高增长被控制。两个优化叠加后，单次操作均摊 O(α(n))，α 是反阿克曼函数，n ≤ 2^65536 时 α(n) ≤ 4，实践中当常数看\n\n复杂度：n 次操作总耗时 O(n·α(n))，接近线性；空间 O(n)。\n\n什么时候想到并查集（判定信号）：\n- 动态连通性：边一条条加进来，随时问「a 和 b 是否连通」——这是并查集对 DFS/BFS 的核心优势：DFS/BFS 是一次性全图判定，并查集支持增量加边实时回答\n- 分组/合并：「朋友的朋友是朋友」「等价关系具有传递性」类问题\n- 判环：无向图加边时 union 返回 false 即出现环（684 冗余连接）\n- 集合计数：初始 n 个集合，每次成功合并集合数 -1，最终集合数即答案（200 岛屿数量：把相邻 1 合并，剩余集合数 = 岛屿数）\n\n经典题型：\n- 200 岛屿数量：把每个 1 与右/下相邻的 1 合并，最后数根的个数\n- 547 省份数量：矩阵给的邻接关系直接合并\n- 684 冗余连接：加边成环的那条边即答案\n- 128 最长连续序列：x 与 x+1 合并后查集合大小\n- Kruskal 最小生成树：边按权排序，依次尝试 union，成环则弃\n\n带权并查集（进阶）：parent 之外再维护 weight[x] = x 到父节点的倍数/距离，find 路径压缩时沿途权值相乘/相加（399 除法求值：w[x] 表示 x = w[x] × root，则 x/y = w[x]/w[y]）。\n\n与 DFS/BFS 的选型：\n- 图一次给全、只问一次连通性 → DFS/BFS 更直接\n- 边动态加入、中途反复查询 → 并查集\n- 需要具体的连通路径（而不只是是否连通） → DFS/BFS（并查集不保存路径）',
    methods: 'find(x) | 查集合代表元（根），路径压缩 | 均摊 O(α(n))\nunite(a, b) | 合并两集合，按秩合并 | 均摊 O(α(n))\nconnected(a, b) | 判断是否同集合 | 均摊 O(α(n))\ninit(n) | 初始化 n 个单元素集合 | O(n)\nunite 返回 false | 两点已同集合 | 可用于判环/判重复边',
    tips: '1. 模板必须零 bug 手写：init/find/unite 三件套，find 一行递归「return parent[x] == x ? x : parent[x] = find(parent[x])」同时完成压缩\n2. 路径压缩与按秩合并只用一个也是 O(α) 级别，但两个都写不费事、面试更稳\n3. 数集合个数：维护 cnt = n，每次 unite 成功 cnt--，避免最后 O(n) 扫根（扫根也行：统计 i == find(i) 的个数）\n4. 网格坐标映射：二维 (i,j) 压成一维 i*n+j，parent 数组开 m*n\n5. 判定信号记「动态加边 + 传递性」；需要具体路径时并查集无能为力，换 DFS/BFS\n6. 带权并查集先想清楚权重的语义（到父的倍率/距离），压缩时怎么合并权值跟着语义走',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 39, name: '排序', category: '算法思想',
    summary: 'O(n log n) 排序（快排/归并/堆排）+ STL sort 用法与自定义比较器；「先排序再看」是大量题目的第一突破口',
    content: '常见排序算法对比（面试必背）：\n- 快速排序：平均 O(n log n)、最坏 O(n²)、空间 O(log n)（递归栈）、不稳定。核心：partition 选基准把数组分成两段，递归处理\n- 归并排序：稳定 O(n log n)、空间 O(n)。核心：递归拆两半，各自排好再线性合并。链表排序首选（148）\n- 堆排序：O(n log n)、原地 O(1) 额外空间、不稳定。核心：建堆后反复取堆顶。C++ STL 无直接堆排序接口，但 priority_queue 就是堆\n- 计数/桶/基数排序：非比较排序，值域受限时可到 O(n)\n\nSTL sort（面试主力，复杂度 O(n log n)）：\n  vector<int> a = {3, 1, 2};\n  sort(a.begin(), a.end());                       // 升序\n  sort(a.begin(), a.end(), greater<int>());       // 降序\n  sort(a.begin(), a.end(), [](int x, int y) {     // lambda 自定义比较器\n      return x > y;                               // 返回 true 表示 x 排在 y 前\n  });\n  // pair / 结构体：依次比较各字段；stable_sort 保持相等元素的相对顺序\n\n比较器三大坑：\n1. 语义是「严格小于」：comp(a,b) 为 true 则 a 在前；写成 >= 会违反严格弱序，RE 或死循环\n2. 比较器内不可修改元素、不可有副作用\n3. 自定义 struct 排序优先用 lambda，比重载 operator< 清晰\n\n「先排序再看」的经典题（为什么排序是突破口）：\n- 15 三数之和：排序后才能固定一个数 + 双指针双向收缩，且跳过重复值只需比较相邻\n- 56 合并区间：按左端点排序后，重叠区间必然相邻，一趟扫描合并\n- 406 根据身高重建队列：按身高降序、同高按序号升序，逐个插入\n- 253 会议室 II：开始/结束时间分别排序，双指针比对\n- 581 最短无序连续子数组：与排序后的数组对照，首个/末个不同的位置即边界\n- 169 多数元素：排序后下标 n/2 处必是多数元素（出现超过一半）\n- 31 下一个排列：从右找第一个降序位，与右边刚大于它的数交换后反转后缀\n\n什么时候不能排序：下标本身携带信息（如子数组的连续性、原顺序有语义）时排序会破坏信息，改用别的前处理。',
    methods: 'sort(first, last) | 区间升序排序，introsort 实现 | O(n log n)\nsort(first, last, comp) | 自定义比较器排序 | O(n log n)\nstable_sort | 相等元素保持原相对顺序 | O(n log n)\nis_sorted / nth_element | 判断有序 / 部分排序（第 k 小就位 + 前 k 小无序） | O(n) / 平均 O(n)\npartial_sort | 只排前 k 个元素 | O(n log k)',
    tips: '1. 区间类问题（合并/覆盖/会议室）几乎总是第一步按左端点排序，排序后重叠的区间必然相邻\n2. 比较器写「严格小于」，写成 >= 是 RE 的经典原因\n3. 双指针 + 排序是固定搭配：排序让「有序」成立，双指针才敢单向移动不回头\n4. 涉及「原下标」的答案（如 581 要返回原数组的边界下标）不能直接在原数组上排，排副本做对照\n5. 需要稳定性（相等元素保持原序）用 stable_sort，普通 sort 是不稳定的\n6. 链表排序用归并（O(1) 额外空间版），数组快排/堆排，各有主场',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 40, name: '归并排序', category: '算法思想',
    summary: '分治拆半 + 线性合并，稳定 O(n log n)；数组版 O(n) 辅助空间、链表版可 O(1)，是 K 路归并和逆序对统计的基础',
    content: '归并排序（Merge Sort）：把序列递归拆成两半，各自排好序后用一个双指针把两个有序段线性合并成一个。分治三步：分解（拆两半）→ 解决（递归排序）→ 合并（merge）。\n\n数组版模板：\n  void mergeSort(vector<int>& a, int l, int r, vector<int>& tmp) {\n      if (l >= r) return;\n      int mid = l + (r - l) / 2;\n      mergeSort(a, l, mid, tmp);\n      mergeSort(a, mid + 1, r, tmp);\n      // 合并 [l,mid] 与 [mid+1,r]\n      int i = l, j = mid + 1, k = l;\n      while (i <= mid && j <= r)\n          tmp[k++] = (a[i] <= a[j]) ? a[i++] : a[j++];  // <= 保证稳定\n      while (i <= mid) tmp[k++] = a[i++];\n      while (j <= r)   tmp[k++] = a[j++];\n      copy(tmp.begin() + l, tmp.begin() + r + 1, a.begin() + l);\n  }\n\n合并两个有序链表（迭代版，148/23 的核心子过程）：\n  ListNode* merge2(ListNode* a, ListNode* b) {\n      ListNode dummy, *t = &dummy;\n      while (a && b) {\n          if (a->val <= b->val) { t->next = a; a = a->next; }\n          else { t->next = b; b = b->next; }\n          t = t->next;\n      }\n      t->next = a ? a : b;\n      return dummy.next;\n  }\n\n为什么链表排序首选归并（148 排序链表）：\n- 快排依赖随机访问（下标 partition），链表没有 O(1) 下标\n- 归并只需求中点（快慢指针）+ 改指针合并，不需要辅助数组，全程 O(1) 额外空间\n- 自底向上迭代版还能避免 O(log n) 递归栈\n\nK 路归并（23 合并 K 个升序链表）：\n- 逐对两两合并：O(Nk)\n- 分治两两合并（每次把 K 路折半配对）：O(N log k)\n- 小顶堆：每次取 K 路头部最小者：O(N log k)，最常写\n\n逆序对统计（归并的经典衍生）：合并时若 a[i] > a[j]（左段元素大于右段元素），则左段 [i, mid] 全部与 a[j] 构成逆序对，一次累加 mid - i + 1 个，边排序边数。\n\n与快排对比：\n- 稳定性：归并稳定（<= 时先取左段），快排不稳定\n- 空间：归并数组版要 O(n) 辅助数组；快排原地 O(log n)\n- 最坏情况：归并恒 O(n log n)；快排最坏 O(n²)（已有序 + 固定基准，随机化基准可规避）\n- 外部排序（大数据放不进内存）：归并是唯一选择，分块排序后多路归并',
    methods: 'mergeSort(l, r) | 递归拆半排序数组 | O(n log n) / 空间 O(n)\nmerge2(a, b) | 双指针合并两个有序链表 | O(m+n) / 空间 O(1)\n分治 K 路归并 | K 路两两折半配对合并 | O(N log k)\n堆式 K 路归并 | 小顶堆每次取 K 路最小头部 | O(N log k)\n逆序对计数 | 合并时左段大于右段则整段计数 | O(n log n)',
    tips: '1. 链表排序一律归并：无下标访问的链表做不了快排 partition\n2. merge2 合并时取 <= 先取左段——这是稳定性的来源，也是逆序对计数 <= 不计、> 才计的边界依据\n3. 23 题三种解法复杂度：逐对合并 O(Nk)、分治和堆都是 O(N log k)，面试写堆最快、写分治最见功底\n4. 逆序对只在「左段元素 > 右段元素」时结算，且一次结算 mid-i+1 个，别数成 1 个\n5. 归并的辅助数组在函数外开一次复用，别每层递归都 vector 一次（会超时）\n6. 迭代版归并（自底向上按步长 1,2,4,8 合并）可以完全去掉递归，链表题常考',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 41, name: 'unordered_set', category: 'STL 容器',
    summary: '哈希集合：O(1) 均摊的去重与存在性查询；与 unordered_map 只差「不存值」，与 set 差在「无序 + 更快」',
    content: 'unordered_set 是基于哈希表的集合容器，只存 key 不存 value，核心用途两件事：去重、O(1) 判断「某元素是否存在」。\n\n与近亲容器的区别：\n- unordered_set vs unordered_map：map 是 key→value 的映射，set 只关心 key 本身（「有没有」，不关心「对应什么」）\n- unordered_set vs set：set 底层红黑树，元素有序、单次操作 O(log n)、迭代按升序；unordered_set 底层哈希表，无序、均摊 O(1)、极端哈希冲突下退化 O(n)（刷题数据量下不用管）\n\n常用 API：\n  unordered_set<int> s;\n  s.insert(3);          // 插入，已存在则无操作\n  s.count(3);           // 1 存在 / 0 不存在（判存在的标准写法）\n  s.erase(3);           // 删除\n  s.size(); s.empty(); s.clear();\n  for (int x : s) {}    // 遍历（顺序不确定）\n\n典型用法（128 最长连续序列）：\n  // 先把所有数丢进集合实现 O(1) 查询，只从「段起点」（x-1 不在集合中）开始向右数\n  int longestConsecutive(vector<int>& nums) {\n      unordered_set<int> s(nums.begin(), nums.end());\n      int ans = 0;\n      for (int x : s) {\n          if (s.count(x - 1)) continue;   // 不是段起点，跳过（保证每段只被数一次）\n          int len = 0;\n          while (s.count(x + len)) len++; // 从起点向右延伸\n          ans = max(ans, len);\n      }\n      return ans;\n  }\n  // 关键：没有集合的话每个数都要 O(n) 扫描找 x+1，整体 O(n²)；有了 O(1) 查询再配合「只从段起点出发」剪枝，整体 O(n)\n\n更多使用场景：\n- 去重后计数（49 字母异位词分组可对排序后的串做 key）\n- 访问标记：图/网格搜索中标记已访问节点（与 visited 数组等价，但节点可以是任意可哈希类型如 string）\n- 快速排除：先查集合确认「绝对不可能」，再走慢路径（如两数之和先查补数是否见过）\n- 字符串键：unordered_set<string> 存词典，逐个判断子串是否在词典中（139 单词拆分）\n\n哈希函数细节：pair/自定义类型默认不可哈希，需要自己写 hash 函数或改用 map（红黑树只需要 operator<）；int/string/vector 有默认哈希，直接用。',
    methods: 'insert(x) | 插入元素（已存在则忽略） | 均摊 O(1)\ncount(x) | 判断存在（1/0） | 均摊 O(1)\nerase(x) | 删除元素 | 均摊 O(1)\nfind(x) | 返回迭代器，end() 表示不存在 | 均摊 O(1)\n迭代器遍历 | 顺序不确定，不能依赖顺序 | O(n)',
    tips: '1. 只需要「存在性」不需要「映射关系」时用 set 而非 map，语义更清晰\n2. 128 的 O(n) 全靠两条：集合 O(1) 查询 + 只从段起点（x-1 不存在）开始数，缺一个都会退化 O(n²)\n3. 需要「有序去重」（按序输出、找最小缺失正数）用 set，unordered_set 无序\n4. pair/自定义 struct 做键要么手写 hash，要么用 set/map（只要 operator<）\n5. 用 unordered_set 当 visited 时，字符串/负数/稀疏下标都能直接存，比 vector<bool> 通用但更慢——值域小且是整数时优先数组标记\n6. 遍历时删除元素的标准写法：it = s.erase(it)，直接 erase(it) 后 it 悬空',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 42, name: '桶排序', category: '算法思想',
    summary: '按值域分桶，桶内各自处理再串接；数据分布均匀时接近 O(n)。计数排序是其特例，频次下标（347）与冷却排班（621）都是桶思想',
    content: '桶排序（Bucket Sort）：把值域区间切成 k 个桶，元素按值落入对应桶，桶内再排序（或直接计数），最后按桶顺序串接。它不是具体算法而是「按值域分而治之」的思想：\n\n- 数据分布均匀 + 桶数恰当：每桶期望 O(n/k) 个元素，桶内排序总代价 O(n)，整体接近 O(n + k)\n- 数据倾斜（全挤进一个桶）：退化 O(n log n)（桶内还是要比较排序）\n- 计数排序是「每个桶只装一种值」的特例：值域为 [0, 有限] 的整数，直接用数组下标当桶，每个桶只计数（75 颜色分类的方法二：统计 0/1/2 个数再重写数组）\n\n经典用法一：频次做桶下标（347 前 K 个高频元素）：\n  // 统计频次后，「频次」是值域有限的整数（1..n），把同频次的元素挂到同一个桶\n  vector<int> topKFrequent(vector<int>& nums, int k) {\n      unordered_map<int, int> cnt;\n      for (int x : nums) cnt[x]++;\n      vector<vector<int>> buckets(nums.size() + 1);   // 桶下标 = 频次\n      for (auto& [x, c] : cnt) buckets[c].push_back(x);\n      vector<int> ans;\n      for (int f = buckets.size() - 1; f >= 0 && ans.size() < k; f--)\n          for (int x : buckets[f]) {\n              ans.push_back(x);\n              if (ans.size() == k) break;\n          }\n      return ans;   // 无比较，O(n)\n  }\n\n经典用法二：按框架排桶（621 任务调度器）：\n  // 出现最多的任务排成 (mx-1) 行，每行 n+1 格（任务+冷却）；\n  // 出现次数等于 mx 的任务种类数 mxcnt 填满最后一列，总时长 = (mx-1)*(n+1) + mxcnt；\n  // 任务多到把冷却填满时取任务总数，两者取大\n  ans = max((int)tasks.size(), (mx - 1) * (n + 1) + mxcnt);\n\n其他桶思想场景：\n- 按出现次数分层后逐层输出（与快速选择对比：桶 O(n) 确定性、快选平均 O(n) 但常数小）\n- 浮点数排序 / 区间均匀数据排序（值域均匀时真正逼近 O(n)）\n- 外部排序与 TopK 大数据场景：先按哈希分桶把大文件拆小，再各桶处理\n\n什么时候想到桶：值域（或频次、数量）是有限小整数，且题目要求线性时间或按频次/数量分层时。',
    methods: '按值分桶 | 元素按值落入区间桶，桶内排序后串接 | 均匀时期望 O(n+k)\n计数排序 | 桶下标即值，只计数不排序 | O(n + 值域)\n频次做下标 | 同频次元素挂同桶（347） | O(n)\n按框架排桶 | 冷却/间隔类问题按 (mx-1)*(n+1) 框架排布（621） | O(n)',
    tips: '1. 判定信号：值域是有限小整数（频次、出现次数、颜色种类），或题目明说「要求线性时间」\n2. 347 桶排序版是确定性 O(n)，堆版 O(n log k)，快选版平均 O(n)——面试三个都能写，桶版最好解释\n3. 621 公式的两部分：(mx-1)*(n+1)+mxcnt 是冷却没被填满的框架长度，任务总数是填满后的下界，取 max\n4. 计数排序要求值域已知且非负整数；负数可整体平移，浮点数/字符串不能直接计数\n5. 桶数取多少：经验值取 n（每桶期望 1 个元素），或按值域均分 k 份\n6. 桶排序不稳定地依赖数据分布，面试先声明「数据均匀时 O(n)」再讲退化情况',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 43, name: '状态机', category: '算法思想',
    summary: '把过程建模成「状态 + 转移边」：定义好状态集合，转移方程就是状态机里的边；股票系列（121/122/309/714）全部同框架',
    content: '状态机（Finite State Machine）思想：把系统在每一时刻的「处境」抽象为有限个状态，事件（新来一个元素/过一天）驱动状态沿转移边跳转。刷题里它有两种形态：\n\n一、状态机 DP（最常用）：DP 的「状态定义」本质就是在画状态机，转移方程就是边。最典型的是股票系列：\n\n121 买卖股票 I（只买卖一次）——两状态：\n  hold[i] = max(hold[i-1], -price[i])            // 今天持有（之前就持有 / 今天买入）\n  cash[i] = max(cash[i-1], hold[i-1] + price[i]) // 今天不持有（早就卖了 / 今天卖）\n\n122 买卖股票 II（可无限次）——两状态，买入成本改成 cash[i-1] - price[i]：\n  hold[i] = max(hold[i-1], cash[i-1] - price[i])\n  cash[i] = max(cash[i-1], hold[i-1] + price[i])\n\n309 含冷冻期——「不持有」分裂成两个状态（冷冻期规则的来源）：\n  hold：持有股票\n  sold：今天刚卖出（明天冷冻，不能买）\n  rest：不持有且非今天卖出（随时可买）\n  hold[i] = max(hold[i-1], rest[i-1] - price[i])   // 买入只能来自 rest！\n  sold[i] = hold[i-1] + price[i]\n  rest[i] = max(rest[i-1], sold[i-1])              // 冷冻一天后 sold 自动转 rest\n\n714 含手续费——122 基础上卖出时多减 fee。188 限定 K 次——状态加一维「已交易次数」。六个股票题是同一个状态机加不同约束，把状态机画出来，转移方程照着边抄即可。\n\n画状态机三步法：\n1. 状态 = 每天结束时所有可能的「持仓/冷却/交易次数」组合，不能有歧义、不能有重叠（309 必须把 sold 和 rest 拆开就是因为两者「明天能不能买」不同）\n2. 边 = 一次合法操作（买入/卖出/持有不动/空闲不动），每条边对应转移方程里的一项 max\n3. 初始值 = 第 0 天各状态的合法取值（hold = -price[0]，cash/rest = 0，非法状态设负无穷），答案 = 最后一天所有「不持有」状态的最大值\n\n二、字符串匹配状态机（10 正则表达式匹配）：DP 状态 f[i][j] = s 前 i 个与 p 前 j 个是否匹配，遇到「*」时在「匹配 0 次」和「多匹配一个」之间跳转，本质是 NFA（非确定有限自动机）的隐式构造；通配符匹配 44 同理。\n\n其它状态机场景：8 字符串转整数（有限状态表枚举「开始/符号/数字/结束/非法」）、LRU/缓存失效策略、协议解析。',
    methods: '定义状态 | 枚举每个时刻所有互斥的处境 | 转移方程的维度\n画转移边 | 每条合法操作一条边，边即 max 的一项 | 照边抄方程\n设初值 | 第 0 天合法状态赋值，非法设 -∞ | 防止非法路径胜出\n取答案 | 最后一天目标状态集合取 max | 通常是「不持有」类状态\n滚动优化 | 状态只依赖上一天 → 压成常数变量 | O(1) 空间',
    tips: '1. 状态必须互斥且完备：309 若不把 sold（刚卖，明天冷冻）和 rest（空闲，可买）拆开，冷冻期规则根本写不进转移\n2. 股票六连题全是一个状态机：约束改了就加状态（冷冻期分裂）或加维度（K 次交易），不要每题重想\n3. 买入的来源受限是冷冻期的全部实现：hold 只能从 rest 转移、不能从 sold 转移，一行搞定\n4. 非法初值用 -∞（如 hold 未初始化就转移），用 0 会让「不买」的假路径胜出\n5. 状态只依赖前一天时，数组压成几个变量：309 的 hold/sold/rest 三个滚动变量即 O(1) 空间\n6. 字符串匹配的「模式串驱动跳转」也是状态机：f[i][j] 的每个 j 就是一个自动机状态，* 是自环边',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 44, name: '记忆化搜索', category: '算法思想',
    summary: '自顶向下的 DP：递归 + 缓存，先查缓存再计算；天然适合树形/区间/状态难以线性枚举的问题（312/279/337）',
    content: '记忆化搜索（Memoization）= 递归 + 缓存：函数入口先查缓存，命中直接返回；算完把结果写进缓存再返回。它与递推 DP 是同一件事的两个方向：\n\n- 递推 DP（自底向上）：从边界条件开始，按拓扑序逐个填表，f[i] 由更小的下标算出\n- 记忆化搜索（自顶向下）：从最终问题 dfs(原问题) 开始，需要什么子问题就递归去算，算过的存起来不再算\n\n两者状态集合、复杂度完全等价（每个状态只算一次），选哪个看哪个好写：\n- 转移天然线性、下标简单 → 递推（背包、路径 DP）\n- 转移是「枚举分割点/树的儿子/图的邻居」、状态不是整数下标 → 记忆化（区间 DP、树形 DP、图上博弈）\n\n标准模板（以 312 戳气球为例，状态是「开区间」）：\n  vector<vector<int>> memo;\n  vector<int> a;                       // 两端补 1 的数组\n  int dfs(int l, int r) {              // 戳破开区间 (l,r) 内所有气球的最大得分\n      if (l + 1 >= r) return 0;        // 区间内没气球\n      int& res = memo[l][r];\n      if (res != -1) return res;       // 缓存命中\n      for (int k = l + 1; k < r; k++)  // 枚举「最后戳」的气球\n          res = max(res, dfs(l, k) + dfs(k, r) + a[l]*a[k]*a[r]);\n      return res;\n  }\n\n模板要点：\n1. int& res = memo[l][r] 引用别名，后面直接写 res，避免反复下标\n2. 缓存初值选一个不可能成为答案的哨兵（0 得分的题用 -1，可能为负的用 INT_MIN，或用哈希 map.count 判存在）\n3. 无后效性由「状态定义」保证：312 定义成开区间 (l,r)，戳谁都不影响外面的气球\n\n为什么 312 适合记忆化而难写递推：转移依赖「枚举最后戳谁」，状态是二维区间；记忆化把「大区间→小区间」的依赖交给递归栈自动处理，递推则要小心按区间长度循环。\n\n更多典型题：\n- 279 完全平方数：记忆化 dfs(n) = min(dfs(n-j*j)) + 1（虽然这题递推更好写）\n- 337 打家劫舍 III：树形 DP，dfs 返回 (选, 不选) 二元组，哈希表<TreeNode*, pair> 做缓存（节点地址做键）\n- 10 正则表达式匹配：dfs(i, j) = s[i..] 与 p[j..] 是否匹配，二维 memo\n- 博弈论（486 预测赢家）：dfs(l, r) 返回「先手在 [l,r] 能取得的最大净分」，天然递归\n\n记忆化 vs 普通递归 vs 递推对比（以 fib 为例）：普通递归 O(2^n)（重叠子问题重复算）、记忆化 O(n)、递推 O(n)。记忆化就是「给递归装缓存，把指数剪成多项式」。',
    methods: 'dfs(state) + memo | 入口查缓存、出口写缓存 | 每状态 O(1) 次\n树形记忆化 | unordered_map<节点指针, 结果> | 树 O(n)\n区间记忆化 | memo[l][r]，按区间自然递归 | O(n²)~O(n³)\n缓存哨兵 | -1 / INT_MIN / map.count 判未算 | 不可与合法答案混淆\n结果别名 | int& res = memo[..] | 防止下标写错',
    tips: '1. 写递归 TLE 了，第一反应就是加 memo——重叠子问题 + 无后效性，两条满足就能记\n2. 缓存哨兵不能撞上合法答案：可能得 0 分的题用 -1，可能为负的用 NaN 型哨兵或改用哈希表判存在\n3. 引用别名 int& res = memo[l][r] 后不要再通过别的路径改 memo，一处的修改即缓存写入\n4. 状态里「跳过的部分」必须从状态定义里不可见（312 的开区间定义是全部精髓），否则后效性导致答案错\n5. 树上记忆化用节点指针做键（unordered_map<TreeNode*, pair<int,int>>），不要试图给节点编号\n6. 面试先说「记忆化 = 自顶向下 DP，与递推等价」，再写代码；能顺手给出递推版的题（279/322）两个都提一句更显功底',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 45, name: '双端队列', category: 'STL 容器',
    summary: 'deque：两端插入删除均 O(1) 的双向开口容器；配合「单调性」变成单调队列（239 滑动窗口最大值），配合哈希变成 LRU',
    content: '双端队列（deque，double-ended queue）：两端都能进出（push_back / push_front / pop_back / pop_front 全 O(1)）的序列容器。刷题中它有两层身份：\n\n一、STL 容器 deque 本体：\n  deque<int> dq;\n  dq.push_back(1); dq.push_front(0);   // 两端插入 O(1)\n  dq.pop_back();  dq.pop_front();      // 两端删除 O(1)\n  dq.front();     dq.back();           // 访问两端 O(1)\n  dq[i];                               // 随机访问 O(1)（分段连续内存，比 list 强）\n与 vector/list 对比：vector 尾部 O(1) 但头部 O(n)；list 两端 O(1) 但随机访问 O(n)、缓存不友好；deque 两端 + 随机访问都行，是「滑动窗口」类问题的标准载体。\n\n二、单调队列（deque 最经典的应用，239 滑动窗口最大值）：\n  // 维护一个「下标递增、对应值严格递减」的双端队列，队头永远是窗口最大值\n  vector<int> maxSlidingWindow(vector<int>& nums, int k) {\n      deque<int> dq;                    // 存下标\n      vector<int> ans;\n      for (int i = 0; i < nums.size(); i++) {\n          if (dq.front() <= i - k) dq.pop_front();          // 1) 队头过期出队\n          while (!dq.empty() && nums[dq.back()] <= nums[i])\n              dq.pop_back();                                // 2) 队尾干不过新元素的出队\n          dq.push_back(i);                                  // 3) 新元素入队尾\n          if (i >= k - 1) ans.push_back(nums[dq.front()]);   // 4) 队头即窗口最大值\n      }\n      return ans;\n  }\n正确性：队头下标最新、值最大；任何比新元素小的旧元素不可能再成为最大值（新元素活得比它们久），放心弹出——每个元素至多入队出队各一次，整体 O(n)。\n\n单调队列适用：滑动窗口最值（239）、定长连续子串/子数组的极值约束问题（1438 绝对差不超过限制的最长子数组：单调队列同时维护最大和最小）。变体：\n- 严格递减 vs 非严格：弹「<=」保严格递减，弹「<」允许重复——按题目对相等元素的语义选\n- 存下标而非值：才能判断过期（239 需要下标算窗口位置）；不需要过期判断时可存值\n\n三、其它双端队列场景：\n- BFS 队列用 deque 承载（push_back / pop_front 即先进先出）；0-1 BFS（边权只有 0/1 的最短路）用 deque 双向入队：权 0 的 push_front、权 1 的 push_back\n- 滑动窗口的最大值/最小值各维护一条单调队列\n- 回文判断/生成（一端进两端出模拟）',
    methods: 'push_back / push_front | 尾部/头部插入 | O(1)\npop_back / pop_front | 尾部/头部删除 | O(1)\nfront() / back() | 访问两端元素 | O(1)\noperator[] | 随机访问（分段连续内存） | O(1)\n单调队列（整体） | 每元素至多进出各一次 | 均摊 O(n)',
    tips: '1. 单调队列四步节奏背下来：过期出队头 → 干不过的出队尾 → 新元素入队 → 队头取答案（239 的 for 循环体就这四行）\n2. 存下标不存值：需要判断过期（窗口滑出）时只有下标能算位置\n3. 弹队尾的条件（<= 还是 <）决定相等元素去留，影响的是「相等时取谁」，按题意定\n4. 单调队列 O(n) vs 堆 O(n log n) vs 平衡树 O(n log n)：滑动窗口定长最值优先单调队列\n5. BFS 用 queue 即可，只有 0-1 BFS 才需要 deque 的 push_front\n6. deque 头部插入是 vector 做不到的 O(1)：「从头部不断补元素、从尾部消费」的结构优先 deque',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 46, name: '字符串匹配', category: '算法思想',
    summary: '在文本串中找模式串：暴力 O(nm)、KMP O(n+m)（next 数组）、Rabin-Karp 滚动哈希；与 10 正则 DP 的关系是把匹配规则泛化',
    content: '字符串匹配（Pattern Matching）：在长度 n 的文本串 s 中查找长度 m 的模式串 p 的出现位置。刷题主线：\n\n一、暴力匹配（BF，Two Pointers）：\n  // 每个对齐位置逐字符比较，失配则起点右移一位重来\n  for (int i = 0; i + m <= n; i++) {      // 对齐起点\n      int j = 0;\n      while (j < m && s[i+j] == p[j]) j++;\n      if (j == m) return i;               // 找到\n  }\n复杂度 O(nm)，但实践中失配通常发生很早，真实速度不差；m、n 小或面试热身直接写它。\n\n二、KMP（O(n+m)，核心是 next 数组）：\nnext[j] = p[0..j] 的最长相等前后缀长度（border）。失配时不回退文本指针 i，而是把模式串「跳到」next 指示的位置继续比——已匹配部分的最长相等前后缀保证了这部分不用重比。\n  // 求 next（模式串自我匹配）\n  vector<int> buildNext(string& p) {\n      vector<int> nxt(p.size() + 1, 0);\n      for (int i = 1, j = 0; i < p.size(); i++) {\n          while (j && p[i] != p[j]) j = nxt[j];\n          if (p[i] == p[j]) j++;\n          nxt[i + 1] = j;\n      }\n      return nxt;\n  }\n  // 匹配\n  int kmp(string& s, string& p) {\n      vector<int> nxt = buildNext(p);\n      for (int i = 0, j = 0; i < s.size(); i++) {\n          while (j && s[i] != p[j]) j = nxt[j];\n          if (s[i] == p[j]) j++;\n          if (j == p.size()) return i - j + 1;   // 首次出现位置\n      }\n      return -1;\n  }\nnext 数组的衍生应用：28 找出字符串中第一个匹配项的下标（KMP 模板题）、459 重复的子字符串（len % (len - next[len]) == 0 判断由循环节构成）、214 最短回文串（next 思想求最长回文前缀）。\n\n三、Rabin-Karp（滚动哈希）：把子串哈希成整数，滑动窗口一次 O(1) 更新哈希（高位乘 base 的幂再进位、减去滑出字符），哈希相等再逐字符核对防碰撞。均摊 O(n+m)，多模式串匹配（把所有模式串哈希进集合）与 1044 最长重复子串（哈希 + 二分长度）是它的主场。\n\n四、通配符/正则匹配（10、44）——匹配规则的泛化：\np 中出现「.」（配任意单字符）和「*」（配零个或多个前一字符）后，确定性跳转失效，改成 DP：f[i][j] = s 前 i 个与 p 前 j 个能否匹配，「*」分「匹配 0 次（吃掉 x*）」与「多匹配一个（s 再进一个、p 不动）」两支转移。可以理解为把 KMP 的自动机换成带「*」自环的非确定自动机，用 DP 而不是指针跳转处理。\n\n选型：精确匹配小规模用暴力；大规模/多次查询用 KMP；多模式串、需要子串哈希判等用 Rabin-Karp；带通配规则一律 DP。',
    methods: '暴力匹配 | 每个起点逐字符比对 | O(nm)\nKMP | next 数组失配跳转，文本指针不回退 | O(n+m)\nRabin-Karp | 滚动哈希 + 窗口滑动 | 均摊 O(n+m)\nnext 数组 | 最长相等前后缀，兼判循环节 | O(m)\n正则/通配 DP | f[i][j] 二维表，* 分零次/多次 | O(mn)',
    tips: '1. KMP 两段代码同构：buildNext 是模式串自己和自己匹配，匹配循环只多一个「j == m 的成功判定」，背一段等于背两段\n2. next 定义有「前缀函数值」与「+1 位移」两派，边界差 1，面试先声明自己用哪套再写\n3. 滚动哈希防碰撞：哈希相等必须逐字符核对，或用双哈希（两个 base/mod）把碰撞概率压到忽略不计\n4. 459 重复子字符串：len 能被 (len - next[len]) 整除即存在循环节，next 的经典衍生\n5. 10 正则匹配初始化别漏：f[0][j] 对 a*b*c* 型模式可能为 true（* 匹配零次）\n6. 字符串下标从 0 还是 1、next 是否整体 +1，是 KMP 代码写崩的头号来源，先小样例 dry-run',
    createdAt: '2026-09-14T10:00:00'
  },
  {
    id: 47, name: '数据结构设计', category: '技巧',
    summary: '设计类题套路：从「操作 + 复杂度要求」倒推数据结构组合（155 最小栈 / 146 LRU / 232 队列实现栈），各结构各取所长',
    content: '设计类题（Design）不考新算法，考「用已有数据结构组合出满足复杂度要求的新结构」。方法论：\n\n第一步：列出每个操作 + 它的复杂度要求（这是全部线索）。\n例 146 LRU：get O(1) + put O(1)。\n\n第二步：按复杂度倒推候选结构：\n- 要求 O(1) 查询 → 哈希表\n- 要求 O(1) 两端/中间插删 → 链表（list，持迭代器时）\n- 要求最值 O(1) → 堆 / 单调栈 / 双栈\n- 要求有序 + 前驱后继 → 平衡树（set/map）\n- 要求前缀统计 → 前缀和 / 前缀树\n\n第三步：单一结构不够就组合，让每个结构干它最擅长的事：\n- 146 LRU = unordered_map（查得快）+ list（排使用顺序）：map 的值存 list 迭代器，get 时 O(1) 定位 + splice 提队头；淘汰从链表队尾发起，节点里反存 key 用于删 map 条目\n- 155 最小栈 = 主栈（栈序）+ 辅助栈（同步维护前缀最小值）：push 时辅栈压 min(x, 辅栈顶)，pop 两栈同弹，getMin 读辅栈顶——「空间换时间 + 冗余维护前缀信息」\n- 232 用栈实现队列 = 两个栈倒腾：in 栈只进、out 栈只出，out 空了把 in 整个倒过去（均摊每元素 O(1)，每元素至多被搬两次）\n- 225 用队列实现栈 = 一个队列，push 后把前面 n-1 个转圈弹到尾部\n- 380 O(1) 插入删除随机取数 = 数组（随机下标）+ 哈希（值→下标）：删除时把末尾元素 swap 补到被删位置，保持数组紧凑\n- 895 最大频率栈 = 频次 → 栈 的桶 + 当前最大频次：push 进 freq+1 的桶并更新 maxFreq，pop 从 maxFreq 桶顶弹\n- 460 LFU = LRU 的频次分桶版：freq → list（同频次内按 LRU），minFreq 随淘汰维护\n\n设计题的共同模式总结：\n1. 冗余维护：一份主数据 + 若干份辅助信息（最小栈的辅栈、LRU 的 map 索引），写操作多花成本换取读操作 O(1)\n2. 双向索引：a 结构存 b 结构的句柄、b 结构反存 a 的键（LRU 的 map ↔ list 节点），删除必须两边同步、且注意先后顺序（先删 map 再 pop 链表，否则 key 取不到）\n3. 均摊分析：232 的倒栈不是每步 O(1) 而是「最坏 O(n)、均摊 O(1)」，每元素被搬次数有上界；面试要会讲均摊而非只会说平均\n4. 哨兵简化：哑头/哑尾/队尾补 0（84 单调栈的封底哨兵），把边界判断并进主循环\n5. 均摊删除：堆里删任意元素难 → 懒删除（标记 + 弹出时跳过）；LFU 的频次桶 → 桶空了才降 minFreq\n\n面试流程：先说「要满足 X 操作 O(1)，单靠 Y 不行，因为 Z；组合 Y 和 W 各司其职」，讲清选型理由再写代码——设计题的分在选型，不在代码量。',
    methods: '倒推选型 | 从操作复杂度要求反查候选结构 | 设计第一步\n组合分工 | map 管查询 + list 管顺序（146） | O(1) 双全\n冗余前缀 | 辅栈同步维护前缀最小值（155） | 空间换时间\n双向索引 | map 存迭代器 + 节点反存 key | 删除两边同步\n均摊分析 | 倒栈每元素至多搬两次（232） | 均摊 O(1)\n懒删除 | 标记删除 + 弹出时跳过 | 堆删任意元素',
    tips: '1. 审题先抄下每个操作的复杂度要求，它是选型的全部依据；达不到的组合直接放弃\n2. 155 最小栈的辅栈可以只压「更小或相等」的值（弹出时值相等才弹），省空间；无脑同步压也可，面试两种都提\n3. 146 的删除顺序铁律：先 mp.erase(dq.back().first) 再 dq.pop_back()，反了 key 就取不到了\n4. 232 讲均摊：元素只在 in→out 单向搬运，每元素至多被搬两次，n 次 push + n 次 pop 总代价 O(n)\n5. 380 随机取数的删除用「尾元素补位」：swap 到被删位置再 pop_back，数组始终无洞\n6. 设计题先讲选型再写码：每个结构一句「它负责什么、为什么别的结构不行」，这是得分点',
    createdAt: '2026-09-14T10:00:00'
  }
];
